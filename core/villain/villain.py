...
from common import *
from settings import Hoaxshell_settings, Core_server_settings, Netcat_settings
from villainSocketPayload import socketPayloadGen
from villain_core2 import initiate_hoax_server, Sessions_manager, Hoaxshell, Core_server, Netcat_multi_listener, Payload_generator
...
def main():
	...
	''' our modded socket gen '''
	sockLoad = socketPayloadGen()
	sockLoadThread = Thread(target=sockLoad.start, daemon=True).start()
	...
	while True:
		...
		try:	
			...
			if verified:				
				...
				cSkt.connect(("127.0.0.1", 64901))
				cSkt.sendall('{"exit": "0"}'.encode("utf-8"))
				cSkt.close()
