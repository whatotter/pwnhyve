...
3.1 chmod that bash file and pwnhyve' main file
4. edit your rc.local file to auto start the isticktoit gadget
4.1. make a systemd service to start pwnhyve
...
confirm? [y/n]
    ...
    subprocess.getstatusoutput("sudo cp ./core/installation/pwnhyveusb /bin/")
    subprocess.getstatusoutput("sudo cp ./core/installation/pwnhyvessh /bin/")
    subprocess.getstatusoutput("sudo chmod +x /bin/pwnhyveusb")
    subprocess.getstatusoutput("sudo chmod +x /bin/pwnhyvessh")
    ...
    with open("/bin/pwnhyveStart", "w") as f:
        f.write(open("./core/installation/startup.sh", "r").read().replace("%cwd%", cwd))

    subprocess.getstatusoutput("sudo chmod +x /bin/pwnhyveStart")

    with open("/etc/systemd/system/pwnhyve.service", "w") as f:
        f.write(open("./core/installation/pwnhyve.service", "r").read().replace("%cwd%", cwd))
        f.flush()

    subprocess.getstatusoutput("sudo systemctl enable pwnhyve.service")
